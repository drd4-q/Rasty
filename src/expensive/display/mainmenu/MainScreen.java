...
// Расчет координат для рисования логотипа в стиле Minecraft
        int xLogo = (windowWidth - logoWidth) / 2; // По центру по горизонтали
        int yLogo = windowHeight / 10; // 10% от высоты окна сверху (примерное положение логотипа Minecraft)
        boolean small = mainWindow.getWidth() < 900 && mainWindow.getHeight() < 900;
        if (small) {
            yLogo += 30; // Дополнительное смещение для маленьких экранов
        }
